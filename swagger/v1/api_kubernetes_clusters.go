...
/*
KubernetesClustersApiService Reports where a cluster&#x27;s control plane would run, per location.
For each location, reports whether the control plane would be hosted by Crusoe or run on VMs in the project, and where it runs on VMs, which VM types are eligible. Creates nothing. Use it to decide where a control-plane capacity check applies before offering cluster creation, and which VM types to check.
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param projectId ID of the project the cluster would be created in.
 * @param optional nil or *KubernetesClustersApiGetClusterPreflightOpts - Optional Parameters:
     * @param "Locations" (optional.Interface of []string) -  Locations to report on, comma-separated. Omit to report on every location.
@return KubernetesClusterPreflightResponse
*/

type KubernetesClustersApiGetClusterPreflightOpts struct {
	Locations optional.Interface
}

func (a *KubernetesClustersApiService) GetClusterPreflight(ctx context.Context, projectId string, localVarOptionals *KubernetesClustersApiGetClusterPreflightOpts) (KubernetesClusterPreflightResponse, *http.Response, error) {
	var (
		localVarHttpMethod  = strings.ToUpper("Get")
		localVarPostBody    interface{}
		localVarFileName    string
		localVarFileBytes   []byte
		localVarReturnValue KubernetesClusterPreflightResponse
	)

	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/projects/{project_id}/kubernetes/clusters/preflight"
	localVarPath = strings.Replace(localVarPath, "{"+"project_id"+"}", fmt.Sprintf("%v", projectId), -1)
	...
	if localVarHttpResponse.StatusCode >= 300 {
		...
		if localVarHttpResponse.StatusCode == 200 {
			var v KubernetesClusterPreflightResponse
			err = a.client.decode(&v, localVarBody, localVarHttpResponse.Header.Get("Content-Type"))
			if err != nil {
				newErr.error = err.Error()
				return localVarReturnValue, localVarHttpResponse, newErr
